...
export default function Announcements({block, pageId, pageType}: AnnouncementsProps) {
  ...
  const listAttr = createDataAttribute({
    id: pageId,
    type: pageType,
  ...
  return (
    <div className="py-10 px-2">
      ...
      <div
        className="flex flex-row justify-start px-12 items-start gap-10 flex-wrap"
        data-sanity={listAttr()}
      >
        {announcements?.map((announcement) => {
          const {category, title, summary, button, image, date} = announcement

          const itemAttr = createDataAttribute({
            id: pageId,
            type: pageType,
          ...
          return (
            <div
              ...
              className="flex flex-col items-start my-4 gap-2 min-w-0 flex-shrink-0"
              data-sanity={itemAttr()}
            >
              <div className="h-30 max-w-[200px] w-full overflow-hidden rounded relative">
                {image && <CoverImage image={image} attribute={''} />}
              </div>
              ...
              <ResolvedLink link={button?.link} className="text-[#f50] underline text-sm">
                {button?.buttonText}
              </ResolvedLink>
            </div>
          )
