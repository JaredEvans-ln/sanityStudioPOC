...
export default function CallToAction({block, pageId, pageType}: CtaProps) {
  const {heading, eyebrow, body = [], button, image, theme} = block
  const {customBackgroundColor, customTextColor, customButtonBgColor, customButtonTextColor} =
    theme || {}
  const themeName = stegaClean(theme?.themeName) || 'light'
  const imageAttr = createDataAttribute({
    id: pageId,
    type: pageType,
    path: `pageBuilder[_key=="${block._key}"].image`,
  })
  ...
  switch (contentAlignment) {
    ...
    case 'mediaFirst':
      switch (orientation) {
        case 'horizontal':
          layoutClasses = 'flex-row-reverse justify-between items-center px-12 gap-4'
          break
        case 'vertical':
  ...
  return (
    ...
    >
      ...
      <div className={cn(orientation === 'horizontal' ? 'w-1/2' : 'w-full')}>
        {image && <CoverImage image={block.image} attribute={imageAttr()} />}
      </div>
    </div>
  )
}
