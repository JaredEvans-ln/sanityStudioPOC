...
export default function ContactForm({block, pageId, pageType}: ContactFormProps) {
  const {title, body, requiredFields = [], optionalFields = [], successMessage} = block
    ...
    return (
      <div className="w-full py-12 px-4 sm:px-6 lg:px-8 flex justify-center items-center min-h-200">
        <div role="status">
          <svg
            aria-hidden="true"
            className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-[#f50]"
            viewBox="0 0 100 101"
            fill="none"
      ...
      <div className="w-full py-12 px-4 sm:px-6 lg:px-8 flex justify-center items-center min-h-200">
        {successMessage && (
          <CustomPortableText
            parentId={pageId}
            parentType={pageType}
            parentPath={`pageBuilder[_key=="${block._key}"].successMessage`}
            className="!text-[#f50]"
            value={successMessage as PortableTextBlock[]}
          />
        )}
      </div>
    )
